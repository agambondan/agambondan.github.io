...
export function generateStaticParams() {
    return getBlogPosts("id").map((post) => ({ slug: post.slug }));
}

export function generateMetadata({ params }: Params): Metadata {
    const post = getBlogPostBySlug(params.slug, "id");
    if (!post) {
    ...
    return {
        ...
        openGraph: {
            ...
            url: `/id/blog/${post.slug}`,
            locale: "id_ID",
        },
        alternates: {
            canonical: `/id/blog/${post.slug}`,
            languages: {
                en: `/blog/${post.slug}`,
...
export default function IndonesianBlogPostPage({ params }: Params) {
    const post = getBlogPostBySlug(params.slug, "id");
    ...
    return (
        <main className='mx-auto max-w-4xl space-y-8'>
            ...
            <article className='markdown-body blog-body markdown-surface rounded-xl border border-violet-200/20 p-6 md:p-8'>
                <MarkdownContent content={post.contentMarkdown} locale='id' />
            </article>
        </main>
    );
}
